...
import { setStoredWebsiteId } from '../../components/WebsiteSelector';
...
// instead of allocating one per rendered metric.
const sharedNumberFormat = new Intl.NumberFormat('en-US');
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        setSelectedWebsite((current) => {
          const preferred =
            stored || current?.id || null;

          const match = preferred
            ? data.find(
                (website) =>
                  website.id === preferred,
              )
            : undefined;
            ...
            <div className="relative min-w-0 flex-1">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  /*
                   * Persist workspace-wide and notify:
                   * AppShell remounts on WEBSITE_EVENT so
                   * this surface and every other surface
                   * resolve the same site with one load.
                   */
                  if (
                    next &&
                    typeof window !== 'undefined'
                  ) {
                    setStoredWebsiteId(next.id);
                  }
                }}
